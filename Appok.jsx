...
<div style={styles.pdfMonthGrid}>
  {calendarCells.map((item, index) => {
    if (item.type === "empty") {
      return <div key={`pdf-empty-${index}`} style={styles.pdfEmptyCell} />;
    }
    ...
    return (
      ...
      >
        ...
        {item.data.holidayName ? (
          <div style={styles.pdfDayHoliday}>{item.data.holidayName}</div>
        ) : null}

        {item.data.solarTerm ? (
          <div style={styles.pdfDaySolarTerm}>{item.data.solarTerm}</div>
        ) : null}

        <div style={styles.pdfDayResult}>{item.result.title}</div>
      </div>
    );
  })}
</div>
